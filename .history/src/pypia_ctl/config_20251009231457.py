...
Examples:
    ::
        ...
        # PIA_PREFERRED_REGIONS=ca-ontario,ca-montreal
        # PIA_RANDOMIZE_REGION=true
        # PIA_PROXY_KIND=socks5
        # PIA_PROXY_HOST=proxy-nl.privateinternetaccess.com
        # PIA_PROXY_PORT=1080

        >>> from pypia_ctl import PiaSettings
        >>> s = PiaSettings()  # doctest: +SKIP
        >>> s.protocol in {"wireguard", "openvpn"}  # doctest: +SKIP
        True
"""
...
class _RegionFilters(BaseModel):
...
class _ProxyPrefs(BaseModel):
    """Default proxy preferences for adapter helpers.

    Args:
        kind: ``"socks5"`` or ``"http"`` (case-insensitive).
        host: Proxy hostname.
        port: Proxy port.
...
class PiaSettings(BaseSettings):
    ...
    Args:
        ...
        preferred_regions: Ordered shortlist for "preferred" strategy.
    ...
    Environment:
        All keys use prefix ``PIA_``. Notable ones:
          ...
          - ``PIA_PROXY_HOST``, ``PIA_PROXY_PORT``, ``PIA_PROXY_USERNAME``, ``PIA_PROXY_PASSWORD``
          - ``PIA_PLUGINS`` (comma list of fully-qualified classes)

    Examples:
        ::
            >>> s = PiaSettings()  # doctest: +SKIP
            >>> isinstance(s.preferred_regions, list)  # doctest: +SKIP
            True
    """
    ...
    region_filters: _RegionFilters = Field(default_factory=_RegionFilters)

    # Proxy defaults used by adapters.* helpers (does not control VPN)
    proxy: _ProxyPrefs = Field(default_factory=_ProxyPrefs)
s